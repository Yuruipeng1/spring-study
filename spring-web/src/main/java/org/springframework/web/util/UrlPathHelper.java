...
public class UrlPathHelper {
	...
	public String getLookupPathForRequest(HttpServletRequest request, @Nullable String lookupPathAttributeName) {
		 ...
		 * 先从请求域中取，因为之前调用getLookupPathForRequest(request)方法之后，将得到的实际路径
		 * 保存到了请求域中
		 */
	...
	public String getContextPath(HttpServletRequest request) {
		...
		if (contextPath == null) {
			//调用原生的getContextPath()方法
			contextPath = request.getContextPath();
		}
		/**
		 * 实际上就是检查contextPath对应的字符串是不是'/'
		 * 这种情况它的上下文路径应该为空，避免后面拼接字符串有两个/
		 */
	...
	protected String determineEncoding(HttpServletRequest request) {
		/**
		 * request原生方法获取当前请求的编码方式（"ISO-8859-1"、"UTF8"）
	...
	public String removeSemicolonContent(String requestUri) {
		/**
		 * removeSemicolonContent属性默认true，表示去掉;后面的内容
		 * 为false表示只去掉Jsessionid
		 * 不管是true还是false，Jsessionid总是被删掉
		 */
		return (this.removeSemicolonContent ?
	...
	public Map<String, String> decodePathVariables(HttpServletRequest request, Map<String, String> vars) {
		/**
		 * true,那么之前获取uri和contextPath的时候就已经解码过了
		 * 此时直接返回，不用解码了
		 */
		if (this.urlDecode) {
			return vars;
		...
		//解码
			...
			//遍历，对每一个变量调用decodeInternal()方法进行解码
