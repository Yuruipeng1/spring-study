...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	 ...
	 */
	/**
	 * 实例化ApplicationContextInitializer接口对象，并调用接口的initialize()方法增强web上下文
	 * @param wac
	 */
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		/**
		 * 表示已处理上下文刷新事件
		 * 当这个监听器没有监听到ContextRefreshedEvent事件的时候
	 ...
	 */
	//创建一个新的 ServletRequestAttributes请求域属性
	@Nullable
	...
	private void initContextHolders(HttpServletRequest request,
		...
		if (localeContext != null) {
			//将创建的LocaleContext保存到LocaleContextHolder中
			LocaleContextHolder.setLocaleContext(localeContext, this.threadContextInheritable);
		}
		if (requestAttributes != null) {
			//将创建的RequestAttributes保存到RequestContextHolder中
			//
			//保存之后，就可以在线程任意地方获取到LocaleContext、RequestAttributes
