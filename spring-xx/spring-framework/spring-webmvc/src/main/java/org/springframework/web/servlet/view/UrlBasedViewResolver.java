...
public class UrlBasedViewResolver extends AbstractCachingViewResolver implements Ordered {
	...
	protected View createView(String viewName, Locale locale) throws Exception {
		...
		if (viewName.startsWith(REDIRECT_URL_PREFIX)) {
			...
			/**
			 * 创建一个重定向视图
			 * isRedirectContextRelative()方法， 判断给定URL是否解释为根路径开始的URL
			 * isRedirectHttp10Compatible()方法， 判断是否应该与HTTP1.0客户端兼容
			 */
		...
		if (viewName.startsWith(FORWARD_URL_PREFIX)) {
			...
			/**
			 * 创建一个jsp视图
			 * 转发视图，因为jsp就是一个Servlet，就是将请求由DispatcherServlet
			 * 转发给视图名对应的Servlet处理
			 */
	...
	protected boolean canHandle(String viewName, Locale locale) {
		...
		/**
		 * 空表示支持处理所有的视图名
		 * 否则判断视图名是否匹配，viewNames可以为*xxx*、xxx*yyy，模糊匹配
		 */
	...
	protected View applyLifecycleMethods(String viewName, AbstractUrlBasedView view) {
		...
		if (context != null) {
			//将视图对象作为一个spring的bean进行初始化
			/**
			 * 回调视图对象实现的Aware接口
			 * 回调所有BeanPostProcessor的before()和after()方法
			 * 回调InitializingBean接口的afterPropertiesSet()方法
			 */
