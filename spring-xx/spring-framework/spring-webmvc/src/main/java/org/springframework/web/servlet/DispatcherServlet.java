...
public class DispatcherServlet extends FrameworkServlet {
	 ...
	 * 2、通过HandlerMapping，将请求映射到处理器（返回⼀个HandlerExecutionChain，它包括⼀个处理器、多个HandlerInterceptor拦截
	 * 器）；
	 * 3、通过HandlerAdapter⽀持多种类型的处理器(HandlerExecutionChain中的处理器)；
	 * 4、调⽤HandlerExecutionChain的interceptor和handler
	...
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
		...
		if (exception != null) {
			...
			//其他异常
				...
				//使用异常解析器解析异常，确定该异常的响应结果,生成一个ModelAndView对象
