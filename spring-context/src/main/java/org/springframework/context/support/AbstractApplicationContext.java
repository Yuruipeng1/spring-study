...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	@Nullable
	// 该容器的环境，这里的环境配置类。关于环境配置，Spring有自己的一套，下面有写介绍。
	private ConfigurableEnvironment environment;

	/** BeanFactoryPostProcessors to apply on refresh. */
	// 在容器启动后的后置处理，这里用的一个集合来保存，证明后置处理有很多
	// 这个集合的数据来源 一般情况是通过程序员调用api注册
	...
	protected void publishEvent(Object event, @Nullable ResolvableType eventType) {
		...
		// 将事件转换成ApplicationEvent，如果event是applicationEvent的实例
		ApplicationEvent applicationEvent;
		if (event instanceof ApplicationEvent) {
			applicationEvent = (ApplicationEvent) event;
		}
		else {
			// event不是applicationEvent的实例，则新建PayloadApplicationEvent实例
	 ...
	 * * ~~~~~~~~~~~
	 * 	 * 返回该容器的内部多播器
	 * 	 * ApplicationEventMulticaster 这个接口可以管理很多个ApplicationListener对象。
	 * 	 * 并将事件发布给这些监听器。
	 * 	 * ~~~~~~~~~~~
	 ...
	 * 	 * 就是将给定的配置文件路径解析成Resource，统一数据结构，
	 * 	 * 方便调用通用的资源解析器进行配置文件解析
	 ...
	 *  * 设置父容器
	 * 	 * 这里不再赘述，关于merge方法，在之前文章有讲解。就是父容器配置会默认继承到子容器，
	 * 	 * 但如果子容器有自己的配置文件，将会覆盖父容器同名配置。
	 * 	 * 使用merge方法后，对父容器做的任何修改都不会映射到子容器中。
	...
	@Override
	// 添加Bean工厂后置处理，这里是通过集合来存储
	// 标准初始化后，修改应用程序上下文的内部bean工厂。 所有bean定义都将被加载，但是尚未实例化任何bean。
	 ...
	 * 3.遍历这个集合当中的类---封装成为一个bd对象---put beanDefinitionMap
	 * 4.遍历beanDefinitionMap---得到bd对象
	 * 5.解析---validate
	 * 6.通过---获得一个bd--class
	 * 7.得到所有构造方法--通过算法推断出合理的一个构造方法
	 * 8.通过这个合理的构造方法反射实例化一个对象
	 * 9.合并bd
	 * 10.提前暴露工厂对象---循环依赖
	 * 11.注入属性---判断是否需要完成属性填充，自动注入
	 * 12.执行部分aware接口
	 * 13.执行部分aware接口 lifecycle callback 注解
	 * 14.lifecy callback 接口
	 * 15.aop 事件
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 设置beanFactory的各个属性 ,告诉子类刷新内部bean工厂。
			...
			try {
				...
				// 大家都知道Finally是释放资源的地方，这里释放的主要是单例bean的相关缓存，因为在
				// 容器中单例bean只需要创建一次，在容器销毁时才销毁。
				resetCommonCaches();
			}
		}
	}
	...
	protected void prepareRefresh() {
		...
		if (this.earlyApplicationListeners == null) {
			...
			// 如果earlyApplicationListeners 监听器不为null,本地应用程序侦听器重置为预刷新状态。
...
//	 * @see org.springframework.web.context.support.WebApplicationContextUtils#initServletPropertySources
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * 先说程序员自己新建的类
		 * 比如你建一个类X，X类实现了ApplicationContextAware
		 * （自动注入）那么X 当中注入的ApplicationContext无效
		 */
		...
		// 表示将来注入BeanFactory类型的依赖则注入 beanFactory
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		//1.beanFactory--为了获取beanFactory当中的BeanFactoryPostProcessor
		//2.getBeanFactoryPostProcessors() --存的是程序员手动通过api往spring容器中提供的
	...
	protected void initLifecycleProcessor() {
		...
		if (beanFactory.containsLocalBean(LIFECYCLE_PROCESSOR_BEAN_NAME)) {
			...
			//将DefaultLifecycleProcessor作为默认的生命周期处理器，注册到BeanFactory中
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		 ...
		 * 		//4、isPrototypeCurrentlyInCreation，如果没得到bean，就去创建一个bean，此时先判断是否是多例。在单例的时候，可能尝试解决循环依赖，如果是多例，循环引用是不允许的。
	 ...
	 * * 在JVM运行时中注册一个名为SpringContextShutdownHook的关闭挂钩，
	 * 	 * 除非关闭该上下文，否则在JVM关闭时将其关闭。
	 * 	 * 代表doClose()进行实际的关闭过程
	...
	protected void doClose() {
		...
		if (this.active.get() && this.closed.compareAndSet(false, true)) {
			...
			// 如果希望的话，让子类做一些最后的清理...,这里和容器的初始化也类似
			onClose();
