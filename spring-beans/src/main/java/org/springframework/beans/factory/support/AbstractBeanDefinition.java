...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	//通过这个函数的逻辑初始化Bean，而不是构造函数或是工厂方法（相当于自己去实例化，而不是交给Bean工厂）
	@Nullable
	...
	// 定义bean的应用。ROLE_APPLICATION：
	// 用户，ROLE_SUPPORT：完全内部使用，与用户无关，ROLE_INFRASTRUCTURE其他复杂的配置。
	 ...
	 */
	/**
	 * 这个方法比较简单，就是判断beanClass是不是一个clazz，
	 * 如果是clazz，就代表BeanDefinition对应的类是否已经被加载到jvm了，
	 * 那么此时就直接使用简单反射了，否则的话，beanClass就是对应类的类名，此时要获取类注解的信息就只有两种方法:
	 *
	 * （1）通过ClassUtils.forName("完全限定名")，将该类加载到虚拟机， 然后使用简单反射获取类中的注解信息。
	 *
	 * （2）第二种方式就复杂了，通过io和ASM直接操作字节码文件而不需要加载到jvm中
	 * @return
	 */
