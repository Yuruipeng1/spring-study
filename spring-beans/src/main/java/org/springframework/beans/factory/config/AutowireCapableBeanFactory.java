...
//该类继承来BeanFactory，是它的拓展接口，能够被那些支持自动装配功能的Bean工厂实现。前提希望现有的Bean实例公开此功能。
//
//该接口不能在常规的应用代码程序中使用
//
//其他框架集成代码能够使用该接口来链接和填充Spring无法控制其生命周期的现有Bean实例。
// 也就是整合在Spring中的第三方框架可以通过该接口来相互调用未被Spring管理的Bean，实现了各个框架以Spring为整合平台的无缝链接。
//
//该接口没有被ApplicationContext系列容器正面实现，因此应用代码几乎没有使用过该接口，
// 该接口可以从ApplitionContext中的getAutowireCapableBeanFactory()方法获取。
//
//您也可以实现{@link org.springframework.beans.factory.BeanFactoryAware}接口，
...
// （BeanFactoryAware有个setBeanFactory(BeanFactory beanFactory)方法，上面说的强转就是使用在这个方法中。）
...
public interface AutowireCapableBeanFactory extends BeanFactory {
	 ...
	 *  * 指示没有外部定义的自动装配的常数。请注意，仍将应用BeanFactoryAware等和注释驱动的注入
	 * 	 * 不装配
	 * @see #createBean
	 * @see #autowire
